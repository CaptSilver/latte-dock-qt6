...
Storage::Storage()
{
    ...
    s_knownErrors << Data::Generic(QLatin1String(Data::Error::APPLETSWITHSAMEID), i18n("Different Applets With Same Id"));
    s_knownErrors << Data::Generic(QLatin1String(Data::Error::ORPHANEDPARENTAPPLETOFSUBCONTAINMENT), i18n("Orphaned Parent Applet Of Subcontainment"));
    s_knownErrors<< Data::Generic(QLatin1String(Data::Warning::APPLETANDCONTAINMENTWITHSAMEID), i18n("Different Applet And Containment With Same Id"));
    s_knownErrors << Data::Generic(QLatin1String(Data::Warning::ORPHANEDSUBCONTAINMENT), i18n("Orphaned Subcontainment"));


    //! Known SubContainment Families
    //! Systray Family
    m_subIdentities << SubContaimentIdentityData{QStringLiteral("Configuration"), QStringLiteral("SystrayContainmentId")};
    //! Group applet Family
    m_subIdentities << SubContaimentIdentityData{QStringLiteral("Configuration"), QStringLiteral("ContainmentId")};
}
...
{
    QString pluginId = group.readEntry(QStringLiteral("plugin"), QString());
...
void Storage::importToCorona(const Layout::GenericLayout *layout)
{
    ...
    QString temp1FilePath = m_storageTmpDir.path() + QStringLiteral("/") + layout->name() + QStringLiteral(".multiple.views");
    ...
    QString tempLayoutFilePath = m_storageTmpDir.path() + QStringLiteral("/") + layout->name() + QStringLiteral(".multiple.tmplayout");
...
QString Storage::newUniqueIdsFile(QString originFile, const Layout::GenericLayout *destinationLayout)
{
    ...
    QString tempFile = m_storageTmpDir.path() + QStringLiteral("/") + currentdestinationname + QStringLiteral(".views.newids");
    ...
    for (const auto &cId : investigate_conts.groupList()) {
        //! Update the options that contain applet ids
        for (const auto &settingStr : ConfigKeys::appletIdListKeys()) {
...
void Storage::syncToLayoutFile(const Layout::GenericLayout *layout, bool removeLayoutId)
{
    ...
    for (const auto containment : *layout->containments()) {
        ...
        if (!removeLayoutId) {
            newGroup.writeEntry(QStringLiteral("layoutId"), QString());
        }
...
void Storage::moveToLayoutFile(const QString &layoutName)
{
    ...
    QString linkedFilePath = Importer::layoutUserFilePath(QLatin1String(Layout::MULTIPLELAYOUTSHIDDENNAME));
    ...
    for(const auto &cId : multiContainments.groupList()) {
        ...
        if (!cname.isEmpty() && cname == layoutName) {
            multiContainments.group(cId).writeEntry(QStringLiteral("layoutId"), QString());
            KConfigGroup singleGroup = singleContainments.group(cId);
            multiContainments.group(cId).copyTo(&singleGroup);
            singleGroup.writeEntry(QStringLiteral("layoutId"), QString());
            singleGroup.sync();
...
QList<Plasma::Containment *> Storage::importLayoutFile(const Layout::GenericLayout *layout, QString file)
{
    ...
    auto newContainments = layout->corona()->importLayout(KConfigGroup(filePtr, QStringLiteral("")));
...
Data::View Storage::newView(const Layout::GenericLayout *destinationLayout, const Data::View &nextViewData)
{
    ...
    QString templateTmpAbsolutePath = m_storageTmpDir.path() + QStringLiteral("/") + QFileInfo(templateFile).fileName() + QStringLiteral(".newids");
...
bool Storage::exportTemplate(const QString &originFile, const QString &destinationFile,const Data::AppletsTable &approvedApplets)
{
    ...
    for (const auto &cId : containments.groupList()) {
        ...
        for (const auto &aId: applets.groupList()) {
            QString pluginId = applets.group(aId).readEntry(QStringLiteral("plugin"), QString());
...
bool Storage::hasOrphanedSubContainments(const Layout::GenericLayout *layout, Data::Warning &warning)
{
    ...
    warning.id = s_knownErrors[QLatin1String(Data::Error::ORPHANEDSUBCONTAINMENT)].id;
    warning.name = s_knownErrors[QLatin1String(Data::Error::ORPHANEDSUBCONTAINMENT)].name;
...
Data::Applet Storage::metadata(const QString &pluginId)
{
    ...
    if (pkg.isValid()) {
        ...
        if (!iconName.startsWith(QLatin1Char('/')) && iconName.contains(QLatin1Char('/'))) {
            data.icon = QFileInfo(pkg.metadata().fileName()).absolutePath() + QStringLiteral("/") + iconName;
        } else {
            data.icon = iconName;
        }
    }
...
Data::AppletsTable Storage::plugins(const QString &layoutfile, const int containmentid)
{
    ...
    for (const auto &cId : containmentGroups.groupList()) {
        ...
        for (const auto &appletId : appletGroups.groupList()) {
            KConfigGroup appletCfg = appletGroups.group(appletId);
            QString pluginId = appletCfg.readEntry(QStringLiteral("plugin"), QString());
...
Data::View Storage::view(const KConfigGroup &containmentGroup)
{
    ...
    vdata.screensGroup = static_cast<Latte::Types::ScreensGroup>(containmentGroup.readEntry(QStringLiteral("screensGroup"), (int)Latte::Types::SingleScreenGroup));
    ...
    vdata.screenEdgeMargin = containmentGroup.group(QStringLiteral("General")).readEntry(QStringLiteral("screenEdgeMargin"), (int)-1);

    int location = containmentGroup.readEntry(QStringLiteral("location"), (int)Plasma::Types::BottomEdge);
    vdata.edge = (Plasma::Types::Location)location;

    vdata.maxLength = containmentGroup.group(QStringLiteral("General")).readEntry(QStringLiteral("maxLength"), (float)100.0);

    int alignment = containmentGroup.group(QStringLiteral("General")).readEntry(QStringLiteral("alignment"), (int)Latte::Types::Center) ;
...
void Storage::updateView(KConfigGroup viewGroup, const Data::View &viewData)
{
    ...
    viewGroup.writeEntry(QStringLiteral("screensGroup"), (int)viewData.screensGroup);
...
QString Storage::storedView(const Layout::GenericLayout *layout, const int &containmentId)
{
    ...
    QString nextTmpStoredViewAbsolutePath = m_storageTmpDir.path() + QStringLiteral("/") + QFileInfo(layout->name()).fileName() + QStringLiteral(".") + QString::number(containmentId) + QStringLiteral(".stored.tmp");
